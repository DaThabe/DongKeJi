...
namespace DongKeJi.Common.Extensions;
...
public static class EnumerableExtensions
{
    ...
    public static IEnumerable<TEntity> SkipAndTake<TEntity>(this IEnumerable<TEntity> values, int? skip = null, int? take = null)
    {
        if (skip is > 0)
        {
    ...
    public static bool Add<T>(this ICollection<T> values, T value, Func<T, bool> matcher)
    {
        if (values.Count > 0)
        {
            if (!values.Any(matcher)) return false;
        }

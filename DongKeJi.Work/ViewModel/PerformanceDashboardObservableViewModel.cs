...
public partial class PerformanceDashboardObservableViewModel(
    ILogger<PerformanceDashboardObservableViewModel> logger,
...
) : LazyInitializeObservableViewModel
{
    #region --上下文属性--
    ...
    private async Task ReloadSalespersonAsync()
    {
        try
        {
            ...
            SelectedSalesperson = SalespersonCollection.FirstOrDefault() ?? StaffViewModel.Default;
        }
        catch (Exception ex)
        {
    ...
    private async Task ReloadCustomerAsync()
    {
        try
        {
            ...
            SelectedCustomer = CustomerCollection.FirstOrDefault() ?? CustomerViewModel.Default;
        }
        catch (Exception ex)
        {
            snackbarService.ShowError(ex);
    ...
    private async Task RemoveCustomerAsync(CustomerViewModel customer)
    {
        try
        {
            ...
            SelectedCustomer = CustomerCollection.TryGetElementWithOffset(index, -1) ?? CustomerViewModel.Default;
    ...
    private async Task ReloadOrderAsync()
    {
        try
        {
            ...
            SelectedOrder = OrderCollection.FirstOrDefault() ?? OrderViewModel.Default;
        }
        catch (Exception ex)
        {
            snackbarService.ShowError(ex);
    ...
    private async Task AddOrderAsync()
    {
        try
        {
            ...
            //更新数据库
            await orderService.AddAsync(orderCreatorVm.Order, orderCreatorVm.Salesperson, SelectedCustomer);
    ...
    private async Task RemoveOrderAsync(OrderViewModel order)
    {
        try
        {
            ...
            SelectedOrder = OrderCollection.TryGetElementWithOffset(index, -1) ?? OrderViewModel.Default;
    ...
    private async Task ReloadConsumeAsync()
    {
        try
        {
            ...
            SelectedConsume = consumes.FirstOrDefault() ?? ConsumeViewModel.Default;
        }
        catch (Exception ex)
        {
    ...
    private async Task RemoveConsumeAsync(ConsumeViewModel consume)
    {
        try
        {
            ...
            SelectedConsume = ConsumeCollection.TryGetElementWithOffset(index, -1) ?? ConsumeViewModel.Default;

...
public partial class StaffPositionDashboardObservableViewModel(
    ILogger<StaffDashboardObservableViewModel> logger,
    ISnackbarService snackbarService,
    IContentDialogService contentDialogService,
    IStaffService staffService,
    IStaffPositionService staffPositionService
) : LazyInitializeObservableViewModel, IStaffPositionDashboardContext
{
    #region --上下文属性--
    ...
    ///     员工
    /// </summary>
    [ObservableProperty] private StaffViewModel _staff  = StaffViewModel.Default;
    ...
    {
        Position = value ?? StaffPositionViewModel.Default;

        Staff = StaffViewModel.Default;
        StaffList.Clear();

        if (Position != StaffPositionViewModel.Default)
        {
    ...
    private async Task ReloadStaffAsync()
    {
        try
        {
            var staffs = await staffService.FindAllByPositionTypeAsync(Position.Type);
